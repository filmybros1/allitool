...
const ToolWorkspace: React.FC = () => {
  ...
  const processFiles = async () => {
    ...
    try {
      ...
      console.error(err);
      alert("An error occurred during high-performance processing. Please check the console for details.");
    } finally {
      setProcessing(false);
    }
  };
  ...
  const handleDownload = () => {
    if (aiResult) {
       ...
       link.href = url;
       link.download = `allitool_summary_${Date.now()}.txt`;
       link.click();
       return;
    }
    ...
    <div className="max-w-6xl mx-auto px-6 py-24">
      <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div className="animate-fade-up">
          <button 
            onClick={() => processedBlobUrl || aiResult ? (setProcessedBlobUrl(null), setAiResult(null), setTargetKB('')) : navigate('/')} 
          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              <h1 className="text-5xl font-black tracking-tighter text-slate-900 leading-tight">
                {processedBlobUrl || aiResult ? 'The Final Result.' : tool.title}
              </h1>
              <p className="text-slate-500 font-medium text-lg">
                {processedBlobUrl || aiResult ? 'Optimized and ready for your next big task.' : tool.description}
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-[4rem] shadow-[0_40px_100px_-20px_rgba(0,0,0,0.06)] border border-slate-50 overflow-hidden flex flex-col md:flex-row min-h-[700px] animate-fade-up">
        <div className={`flex-grow p-12 flex flex-col ${files.length > 0 ? 'bg-slate-50/20' : ''}`}>
          ...
          {processedBlobUrl || aiResult ? (
            <div className="flex flex-col h-full animate-in fade-in slide-in-from-bottom-8 duration-700">
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 h-full">
                <div className="lg:col-span-8 flex flex-col">
                  {aiResult ? (
                     <div className="flex-grow bg-white rounded-[3.5rem] p-12 border border-slate-100 shadow-inner overflow-y-auto mb-10 prose prose-slate max-w-none scrollbar-thin scrollbar-thumb-indigo-200">
                        <div className="flex items-center gap-4 mb-8">
                           ...
                           <h3 className="text-2xl font-black text-slate-900">AI Intelligence Summary</h3>
                        </div>
                    ...
                    <div className="flex-grow bg-slate-100 rounded-[3.5rem] overflow-hidden mb-10 border border-slate-100 relative group min-h-[500px] flex items-center justify-center shadow-inner">
                      {processedMimeType === 'application/pdf' ? (
                        <iframe 
                          src={`${processedBlobUrl}#toolbar=0`} 
                          className="w-full h-full border-none rounded-[3.5rem]"
                          title="PDF Preview"
                        />
                      ) : (
                        <div className="relative p-12 flex items-center justify-center w-full h-full">
                          {isIterating && (
                             <div className="absolute inset-0 z-10 flex items-center justify-center bg-white/60 backdrop-blur-md rounded-[3.5rem]">
                                <div className="flex flex-col items-center gap-4">
                                  ...
                                  <p className="text-indigo-600 font-black text-xs uppercase tracking-widest">Optimizing Fidelity...</p>
                                </div>
                             </div>
                          )}
                          <img src={processedBlobUrl!} className="max-w-full max-h-[550px] object-contain drop-shadow-2xl rounded-2xl transition-all duration-500" alt="Processed preview" />
                          {multiProcessedFiles.length > 1 && (
                            <div className="absolute top-6 right-6 bg-slate-900/80 backdrop-blur-md text-white text-xs font-black px-5 py-2.5 rounded-full border border-white/20 shadow-xl">
                              + {multiProcessedFiles.length - 1} More Pages
                            </div>
                          )}
                        </div>
                      )}
                      <div className="absolute inset-0 pointer-events-none border-[12px] border-transparent group-hover:border-indigo-500/5 transition-all rounded-[3.5rem]"></div>
                    </div>
                  )}
                </div>
                ...
                <div className="lg:col-span-4 flex flex-col gap-8">
                  {((toolId === ToolType.IMG_COMPRESS || toolId === ToolType.COMPRESS) && processedSize && files[0]) && (
                    <div className="p-8 bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-[2.5rem] text-white flex flex-col gap-8 shadow-2xl shadow-indigo-500/30 group">
                      <div className="flex items-center gap-5">
                        <div className="w-14 h-14 bg-white/20 backdrop-blur-xl rounded-2xl flex items-center justify-center border border-white/10 group-hover:rotate-6 transition-transform">
                          <svg className="w-7 h-7" fill="currentColor" viewBox="0 0 24 24"><path d="M19 14l-7 7m0 0l-7-7m7 7V3"/></svg>
                        </div>
                        <div>
                          <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-80">Efficiency Metrics</p>
                          <h4 className="text-3xl font-[1000]">{Math.max(0, Math.round((1 - (processedSize / files[0].file.size)) * 100))}% Smaller</h4>
                        </div>
                      </div>
                      <div className="flex justify-between border-t border-white/20 pt-6">
                        ...
                        <div className="text-right">
                          <p className="text-[10px] font-black uppercase tracking-[0.2em] opacity-60">Optimized</p>
                  ...
                  {toolId === ToolType.IMG_COMPRESS && (
                    <div className="p-8 bg-white rounded-[2.5rem] border border-slate-100 shadow-xl shadow-slate-200/50 flex flex-col gap-6 transform hover:-translate-y-1 transition-all">
                      <div className="flex items-center gap-3">
                        ...
                        <h4 className="text-lg font-black text-slate-900">Custom Target KB</h4>
                      </div>
                      <p className="text-xs text-slate-500 font-bold leading-relaxed">
                        Need to fit a specific upload limit? Enter your target size and we'll handle the rest.
                      </p>
                      <div className="flex gap-2">
                        ...
                        <button 
                          ...
                          className="bg-slate-900 text-white p-4 rounded-2xl hover:bg-indigo-600 disabled:opacity-30 transition-all active:scale-95 shadow-lg shadow-indigo-500/10"
                        >
                  ...
                  <div className="mt-auto flex flex-col gap-4">
                    <button 
                      ...
                      className={`w-full py-8 rounded-[2.5rem] font-black text-2xl text-white shadow-2xl transition-all active:scale-[0.98] ${tool.color} hover:shadow-indigo-500/30 hover:brightness-110 flex items-center justify-center gap-4`}
                    >
                      {aiResult ? 'Export Text' : 'Download Now'}
                    ...
                    <button 
                      ...
                      className="w-full py-6 rounded-[2rem] font-black text-lg text-slate-500 bg-white border border-slate-200 hover:bg-slate-50 hover:text-slate-900 transition-all active:scale-[0.98]"
                    >
                      Start a New Task
                    </button>
                  </div>
            ...
            >
              ...
              <h3 className="text-4xl font-[1000] text-slate-900 mb-4 tracking-tight">Select your documents</h3>
              <p className="text-slate-400 font-black uppercase tracking-[0.2em] text-sm">Drag files here or click to browse</p>
              <input 
                type="file" 
            ...
            <div className="flex flex-col h-full animate-fade-up">
              <div className="flex justify-between items-center mb-12">
                <div>
                  <h3 className="text-3xl font-[1000] text-slate-900">{files.length} {files.length === 1 ? 'File' : 'Files'} Uploaded</h3>
                  {(tool.id === ToolType.MERGE || tool.id === ToolType.IMAGE_TO_PDF) && <p className="text-sm text-indigo-500 font-black uppercase tracking-widest mt-2">Reorder items for best results</p>}
                </div>
                <button 
                  onClick={() => fileInputRef.current?.click()}
              ...
              <div className="mt-auto pt-12 border-t border-slate-100 flex justify-center relative">
                {processing && (
                  <div className="absolute -top-12 left-1/2 -translate-x-1/2 bg-indigo-600 text-white px-6 py-2 rounded-full text-xs font-black uppercase tracking-widest shadow-lg animate-bounce">
                    Analyzing Document...
                  </div>
                )}
                <button 
                  ...
                  className={`w-full md:max-w-xl py-9 rounded-[3rem] font-black text-2xl text-white shadow-2xl transition-all active:scale-[0.98] flex items-center justify-center gap-5 ${
                    processing ? 'bg-slate-300 cursor-not-allowed' : `${tool.color} hover:shadow-indigo-500/40 hover:scale-[1.01] hover:brightness-110`
                  }`}
                >
                  {processing ? (
                    <>
                    ...
                    <>
                      {toolId === ToolType.AI_SUMMARY ? 'Generate AI Intelligence' : 'Start Processing'}
